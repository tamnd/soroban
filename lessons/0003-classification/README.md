...
At lr = 500 the naive version survives until step 12 before dying the same death. The subtract-max version, the one the softmax page teaches, finishes both runs, and its final printed loss is -0.000000000, which is two floats lessons in disguise. The true loss is never zero, but the score gaps are now so wide that the true class's probability rounds to exactly 1.0 in float64. And the stray minus sign is $`-\ln(1.0)`$ landing on IEEE's negative zero, a float equal to zero that remembers the sign of the operation that made it. A printed zero is a claim about the printer's precision, not about the mathematics.
