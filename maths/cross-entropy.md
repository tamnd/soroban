...
The same loss reappears the moment a model predicts symbols instead of classes. A [bigram language model](bigram.md) scores each next-symbol prediction with exactly this $-\ln(p_{\text{true}})$, one term per position, averaged over the whole text; the only difference is that its probabilities come from a count table rather than a softmax. So the loss lesson 0003 built for three temperature classes is the loss every language model in this repo is trained on, unchanged.

There is a second reading of $p_k - y_k$ that lesson 0006 leans on. Sum this gradient over a whole batch of examples that share the same input row and the $y_k$ terms add up to the observed counts of each outcome, while the $p_k$ terms add up to the model's predicted counts, so the row's gradient is (predicted frequency) minus (observed frequency). It is zero exactly when the model's probabilities equal the frequencies in the data, so training a softmax by this loss is a machine for matching observed frequencies, and a neural [bigram model](embedding.md) trained this way lands on the same count table lesson 0005 got by tallying.
